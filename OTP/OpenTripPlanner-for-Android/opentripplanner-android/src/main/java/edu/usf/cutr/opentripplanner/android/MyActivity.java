...
public class MyActivity extends FragmentActivity implements OnFragmentListener{
	...
	private boolean subscribingT = false;
	public boolean isSubscribingT() { return subscribingT; }
	public void setSubscribingT(boolean s) { subscribingT = s; }
	
	private boolean subscribingA = false;
	public boolean isSubscribingA() { return subscribingA; }
	public void setSubscribingA(boolean s) { subscribingA = s; }
	...
	// Event handler for the trip subscriber button.
	public void subscribeTrip(View view) 
	{
		Button item = (Button)view;
		if (!subscribingT)
		{
			mainFragment.subscribe2Trip();
			item.setText("Unsubscribe to trip updates!");
		}
		else
		{
			mainFragment.unsubscribe2Trip();
			item.setText("Subscribe to trip updates!");
		}
	}
	
	// Event handler for the alert subscriber button.
	public void subscribeAlerts(View view) 
	{
		Button item = (Button)view;
		if (!subscribingA)
		{
			mainFragment.subscribe2Alerts();
			item.setText("Unsubscribe to trip alerts!");
		}
		else
		{
			mainFragment.unsubscribe2Alerts();
			item.setText("Subscribe to trip alerts!");
		}
	}
	
	// Event handler for My Trips (fetched from MongoDB)
	public void myTripHistory(View view)
	{
		mainFragment.getTripHistory();
	}
...
//				Log.v(TAG, "CHOOSE CONTACT RESULT OK");
	...
	public Fragment directionFragment;
	...
	public void onSwitchedToDirectionFragment() {
		...
		directionFragment = new DirectionListFragment();
