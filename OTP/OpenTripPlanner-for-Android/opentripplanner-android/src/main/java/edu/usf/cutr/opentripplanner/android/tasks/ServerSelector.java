...
//		Toast.makeText(activity.getApplicationContext(), shown, Toast.LENGTH_SHORT).show();
	...
	protected void onPostExecute(Long result) {
		...
		if (selectedServer != null) {
			...
			{
				String actualCustomServer = "http://itract.cs.kau.se:8081/proxy/api/otp/ws";
				ServerChecker serverChecker = new ServerChecker(activity, context, ServerSelector.this, false);
				serverChecker.execute(new Server(actualCustomServer, context));
				...
				builder.setItems(items, new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int item) {
						...
						if(items[item].equals(context.getResources().getString(R.string.custom_server_name))) {
							...
							String actualCustomServer = "http://itract.cs.kau.se:8081/proxy/api/otp/ws";//prefs.getString(PREFERENCE_KEY_CUSTOM_SERVER_URL, "");
