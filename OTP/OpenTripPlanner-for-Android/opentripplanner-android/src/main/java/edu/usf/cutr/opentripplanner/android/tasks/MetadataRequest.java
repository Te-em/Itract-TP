...
public class MetadataRequest extends AsyncTask<String, Integer, ItractGraph> {
	...
	protected ItractGraph doInBackground(String... reqs) {
		int count = reqs.length;
		Log.d("test", "in metadata");
		//for (int i = 0; i < count; i++) {
			String serverURL = reqs[0];
			//metadata = requestMetadata(serverURL);
			// publishProgress((int) ((i / (float) count) * 100));
		//}
		ItractGraph g = new ItractGraph();
		try {
			g.parse(serverURL);
		...
		Log.d("test", "meta");
		if (!g.isSuccessful()) cancel(true);
		return g;
	}

	protected void onPostExecute(ItractGraph g){//GraphMetadata metadata) {
		if (activity.get() != null){
			try{
				if (progressDialog != null && progressDialog.isShowing()) {
					progressDialog.dismiss();
		...
		if (g != null) {
			...
			callback.onMetadataRequestComplete(g);
		} else {
