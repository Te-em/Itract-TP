...
public class ItineraryDecrypt {
	...
	private void convertToDirectionList(){
		...
		for(Leg leg: legs){
			index++;
			setTotalDistance(getTotalDistance() + leg.getDistance());
	...
	private Direction decryptNonTransit(Leg leg){
		...
		//		Main direction
		Place fromPlace = leg.getFrom();
		Place toPlace = leg.getTo();
...
//		double duration = DateTimeConversion.getDuration(leg.startTime, leg.endTime);
		double totalDistance = leg.getDistance();
	...
	//		distance = Double.valueOf(twoDForm.format(distance)); -->VREIXO
			...
			String absoluteDirString = step.direction;//= getLocalizedAbsoluteDir(absoluteDir, applicationContext.getResources());
	...
	private ArrayList<Direction> decryptTransit(Leg leg){
		...
		String headsign = leg.getHeadsign();
		String agencyId = leg.agencyId;
		String routeShortName = leg.getRouteShortName();
		String routeLongName = leg.getRouteLongName();
		String boardRule = leg.boardRule;
		String alignRule = leg.alightRule;
		long startTime = (Long.parseLong(leg.getStartTime()) + leg.departureDelay);
		long endTime = (Long.parseLong(leg.getEndTime()) + leg.arrivalDelay);
		...
		offDirectionText += applicationContext.getResources().getString(R.string.step_by_step_transit_get_off) + " " + serviceName + " " + mode + " " + route + ConversionUtils.getTimeWithContext(applicationContext, leg.getAgencyTimeZoneOffset(), endTime, true) + "\n";
		...
		onDirectionText += applicationContext.getResources().getString(R.string.step_by_step_transit_get_on) + " " + serviceName + " " + mode + " " + route + ConversionUtils.getTimeWithContext(applicationContext, leg.getAgencyTimeZoneOffset(), startTime, true) + "\n";
	...
	public double getTotalTimeTraveled() {
		...
		String startTimeText = legStart.getStartTime();
		Leg legEnd = legs.get(legs.size()-1);
		String endTimeText = legEnd.getEndTime();
